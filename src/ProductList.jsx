...
import { addItem } from './CartSlice';      // ensure path is correct
...
export default function ProductList({ onHomeClick = () => {} }) {
  ...
  const [addedToCart, setAddedToCart] = useState({}); // { [plant.name]: true }

  // Total quantity in cart (supports either qty or quantity fields)
  const calculateTotalQuantity = () => {
    if (!Array.isArray(cartItems)) return 0;
    return cartItems.reduce((total, item) => {
      const q = typeof item.qty === 'number' ? item.qty : (item.quantity ?? 1);
      return total + q;
    }, 0);
  };
  ...
  const handleAddToCart = (plant) => {
    dispatch(addItem(plant)); // <-- integrate Redux add
      ...
      <div className="navbar" style={styleObj}>
        <div className="tag">
          <div className="luxury">
            <img src="https://cdn.pixabay.com/photo/2020/08/05/13/12/eco-5465432_1280.png" alt="Paradise Nursery Leaf" />
        ...
        <div style={styleObjUl}>
          <div><span style={styleA}>Plants</span></div>

          <div style={{ position: 'relative' }}>
            <a href="#" onClick={handleCartClick} style={styleA} aria-label={`Open cart with ${cartQty} items`}>
      ...
      {!showCart ? (
        <div className="product-grid">
          {plantsArray.map((section) => (
            <div className="product-section" key={section.category}>
              <h2 className="product-section-title">{section.category}</h2>
